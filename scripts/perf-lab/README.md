...
- `nodejs-orm` - [NestJS 11 + TypeORM](../../nodejs) on Node.js, ORM entities + relation loading
- `nodejs-sql` - [NestJS 11 + raw SQL](../../nodejs) on Node.js, no ORM (same binary as `nodejs-orm`, `QUERY_MODE=sql`)
- `rust-orm` - [Axum + SeaORM](../../rust) on Rust, ORM entities + relation loading
- `rust-sql` - [Axum + sqlx](../../rust) on Rust, raw SQL (no ORM) (same binary as `rust-orm`, `QUERY_MODE=sql`)
...
**Default:** All runtimes except `spring3-jvm-aot`, `spring4-jvm-aot`, `nodejs-orm`, `nodejs-sql`, `rust-orm`, `rust-sql`, `go-sql` and `go-orm` are tested. To include the AOT variants, the Node.js runtime, the Rust runtimes, or the Go runtimes, pass them explicitly via `--runtimes`.

> [!NOTE]
> `nodejs-orm` and `nodejs-sql` require Node.js and npm on the benchmark host, and build the same output - only the `QUERY_MODE` environment variable set in each runtime's `runCmd` differs. Unlike the JVM runtimes there is no framework version to pin, so `--quarkus-version`/`--springboot*-version` have no equivalent; dependency versions come from `nodejs/package-lock.json`. Use `--node-args` to set runtime flags (the default `--max-old-space-size=512` is the analogue of the JVM runtimes' `-Xmx512m`). A single Node.js process only saturates one core, so its throughput is not directly comparable to the JVM runtimes at `--cpus-app` widths greater than 1 — see [nodejs/README.md](../../nodejs/README.md).

> [!NOTE]
> `rust-orm` and `rust-sql` require a Rust toolchain (cargo/rustc) on the benchmark host, and build the same binary - only the `QUERY_MODE` environment variable set in each runtime's `runCmd` differs. Same as `nodejs-orm`/`nodejs-sql`, there is no framework version to pin - dependency versions come from `rust/Cargo.lock`. There is no equivalent of `--jvm-args`/`--node-args`: Rust binaries have no runtime flags analogous to `-Xmx`/`--max-old-space-size` to set. See [rust/README.md](../../rust/README.md) for the module's design notes, including why `orm` (SeaORM) is the default query mode rather than `sql`.
