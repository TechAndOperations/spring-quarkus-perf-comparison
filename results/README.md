...
Stacks measured: **Quarkus 3** (with a virtual-threads variant and a Spring-compatibility
variant), **Spring Boot 3 and 4**, **NestJS**, **Axum** (Rust) and **net/http** (Go). Each
comes in several runtime flavours — JVM, GraalVM native, AOT, Leyden, virtual threads
depending on the case — for around thirty entries in `scripts/perf-lab/main.yml`.
...
**Warmup's tail latencies are in a different league.** On a `quarkus3-virtual` run, the
warmup phase peaks at 2.11 s and sits at 1.82 s at p99.99, versus 68 ms and 59 ms on the
measured phase right after — a factor of 30.

**Inter-iteration dispersion clearly separates the two families.** Under identical
settings, `quarkus3-native` varies by 0.8% between iterations, where `quarkus3-virtual`
varies by 9.3%. On JIT runtimes, a gap of under ~10% between two configurations is
therefore not necessarily significant — several comparisons in this archive fall under that
threshold.
...
`--max-old-space-size=512`. Each runtime therefore has only one configuration here, whereas
the 2-core set summarises a memory sweep.
...
The four Java runtimes retain **56.5% ± 0.3** of their 2-core throughput, except
`spring4-native` at 51.2%. A slightly super-linear scale-up: the second core wasn't fully
exploited, consistent with load being split between application processing and waiting on
PostgreSQL.
...
- **The reported branch is wrong** when `--repo-url` is a local path: the pipeline does a
  `cp -R` of the working copy without a `checkout`, so `config.repo.branch` doesn't reflect
  what was actually measured.
- **Instrumentation depth changed on 2026-08-10.** The `go`, `nodejs` and `rust` modules
  received OTel metrics and logs that day; their earlier numbers are lighter and aren't
  comparable to later ones. The Quarkus and Spring modules are unaffected.
