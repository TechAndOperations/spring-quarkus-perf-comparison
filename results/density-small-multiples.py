...
The latency column plots p50-to-p99 as a range mark (hollow dot at p50, filled dot at
p99, joined by a line) rather than the mean density-cpu-scatter.py uses - a runtime
whose median is fast but whose tail is long looks identical to a uniformly-mid
runtime under "mean alone"; the range makes that visible.

Density and latency columns are logarithmic (density spans a factor of 134 rust-orm
to nodejs-orm; the p50-p99 range spans a comparable spread). CPU stays linear - only
...
ROWS = [
    ("rust-orm", 77.74, 94.7, 4.98, 111.28, "—"),
    ("go-orm", 38.80, 118.7, 13.70, 87.47, "—"),
...
def build():
    ...
    for i, ((title, kind_, _values), (skind, lo, hi, ticks)) in enumerate(zip(PANELS, panel_scales)):
        ...
        for rt, density, cpu, p50, p99, note in ROWS:
            fam, shape = kind(rt)
            y = row_y[rt]
            if i == 0:
    ...
    o.append(
        ...
        f"○ p50 · ● p99, joined by a line</text>"
    )
